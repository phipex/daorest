...
public class ClienteResource {
    ...
    public ResponseEntity<ClienteDTO> createCliente2(@Valid @RequestBody ClienteDTO clienteDTO) throws URISyntaxException {
        ...
        ClienteDTO result = clienteService.save2(clienteDTO);
        log.debug("cliente guardado: {}",result);
        return ResponseEntity.created(new URI("/api/clientes/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(ENTITY_NAME, result.getId().toString()))
            .body(result);
    }
    ...
    @Timed
    public ResponseEntity<ClienteDTO> updateCliente2(@Valid @RequestBody ClienteDTO clienteDTO) throws URISyntaxException {
        log.debug("REST request to update Cliente : {}", clienteDTO);
        if (clienteDTO.getId() == null) {
            return createCliente(clienteDTO);
        }
        ClienteDTO result = clienteService.update2(clienteDTO);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, clienteDTO.getId().toString()))
            .body(result);
    }
